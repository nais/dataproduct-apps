...
from dataproduct_apps.crd import (
    TokenX,
    Rules,
    External,
    Inbound,
    ...
    Application(
        ...
        spec=ApplicationSpec(
            ...
            observability=Observability(
                autoInstrumentation=AutoInstrumentation(enabled=True),
                logging=Logging(destinations=[Destination(id="loki")]),
            ),
            accessPolicy=AccessPolicy(
                inbound=Inbound(
                    rules=[
                        ...
                        Rules(
                            application="app2", namespace="team2", cluster="cluster2"
                        ),
                    ]
                ),
                outbound=Outbound(
                    external=[External(host="external-application.example.com")],
                    rules=[
                        Rules(application="app1"),
                        Rules(
                            application="app2", namespace="team2", cluster="cluster2"
...
TEST_DATA_TOPIC_ACCESS = [
    TopicAccessApp(
        ...
        topic="topic1",
        access="read",
        app=AppRef(namespace="aura", name="basta"),
    ),
    TopicAccessApp(
        pool="pool",
        team="team2",
    ...
    TopicAccessApp(
        ...
        topic="topic2",
        access="write",
        app=AppRef(namespace="aura", name="*"),
    ),
    TopicAccessApp(
        pool="pool",
        team="team2",
...
TEST_DATA_SQL_INSTANCES = [
    SqlInstance(
        metadata=ObjectMeta(labels={"app": "basta"}),
    ...
    SqlInstance(
        metadata=ObjectMeta(labels={"app": "babylon"}),
        spec=SqlInstanceSpec(
